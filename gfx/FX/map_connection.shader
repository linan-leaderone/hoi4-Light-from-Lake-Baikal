...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			if (currentP.x > max(p1.x, p2.x) || currentP.x < min(p1.x, p2.x)) {
                dist = max(dist, min(abs(p1.x - currentP.x), abs(p2.x - currentP.x)));
            }

            float4 toRet = float4(vFirstColor.r, vFirstColor.g, vFirstColor.b, 1);
			float pointDist = distance(p1, currentP) + (vSecondColor.b - 0.5f) * 2.5f * dist; 
			float modVal = mod(pointDist, gap);

			float gapSize = gap / 2.f;

			float theMin = max(modVal, gapSize - modVal);
			float sat = saturate(theMin / gapSize);
			sat = pow(sat, 10);
			toRet = float4(toRet.r * sat, toRet.g * sat, toRet.b * sat, 1);
